...
from db_docker_connector import *
...
def config_rest_api():
    """
    Function which returns the content of config table
    :return: JSON file + return code
    :rtype: JSON file + int
    """
    try:
        all_tests = db_get_all_tests_config()
        return {'status': 'ok', 'all tests': all_tests}, 200
    except ValueError as e:
        return {'status': 'error', 'reason': 'no tests'}, 500
    except pymysql.err.Error  as e:
        return {'status': 'error', 'reason': str(e)}, 500

@app.route('/get_max_id')
def get_max_id_rest_api():
    """
    Function which returns the maximum user id found in users table
    :return: JSON file + return code
    :rtype: JSON file + int
    """
    try:
        max_user_id = db_get_max_user_id()
        return {'status': 'ok', 'max_id': max_user_id}, 200
    except ValueError as e:
        return {'status': 'error', 'reason': 'no max id'}, 500
    except pymysql.err.Error  as e:
        return {'status': 'error', 'reason': str(e)}, 500
